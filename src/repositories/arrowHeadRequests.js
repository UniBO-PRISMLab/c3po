...
const queryService = async (thing) => {
  ...
  const arrowHeadRequest = axios.post(
    `${arrowHeadHost}/serviceregistry/query`,
    payload,
    { headers }
  );
  return arrowHeadRequest
    .then((response) => {
...
const registerService = async (thing) => {
  ...
  const headers = headerFactory.post();
  const arrowHeadRequest = axios.post(
    `${arrowHeadHost}/serviceregistry/register`,
    payload,
    { headers }
  );
  return arrowHeadRequest
...
const getAllServices = () => {
  ...
  const arrowHeadRequest = axios.get(
    `${arrowHeadHost}/serviceregistry/mgmt?direction=ASC&sort_field=id`,
    { headers }
  );
  return arrowHeadRequest
    .then((res) => {
      poolingMetadata.setTimestamp(new Date().toUTCString());
      metrics.setGeneralArrowheadCallTimestamp(new Date().getTime());
...
const deleteService = async (thing) => {
  ...
  const arrowHeadRequest = axios.delete(
    `${arrowHeadHost}/serviceregistry/mgmt/${thing.id}`,
    { headers }
  );
  return arrowHeadRequest
    .then((response) => response.data)
