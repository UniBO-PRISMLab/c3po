...
//TODO: refactor this function. There is a lot of code duplication
const createThing = async (td, serviceUrl) => {
  ...
  for (const key in thing.properties) {
      ...
      thing.setPropertyWriteHandler(key, async (input) => {
        ...
        logger.info(
          `performing PUT request for ${key} property of ${
            td.title
          }  at ${`${serviceUrl}/${replacer.formatUri(
            key
          )}`}`
        );
    ...
    if (key.includes("patch-")) {
      thing.setPropertyWriteHandler(key, async (input) => {
        ...
        const propertyRequest = axios.patch(
          `${serviceUrl}/${replacer.formatUri(key)}`,
          input,
          { headers }
        );
        logger.info(
          `performing PATCH request for ${key} property of ${
            td.title
          }  at ${`${serviceUrl}/${replacer.formatUri(
            key
          )}`}`
      ...
      thing.setPropertyReadHandler(key, async () => {
        ...
        logger.info(
          `performing GET request for ${key} property of ${
            td.title
          } Thing at ${`${serviceUrl}/${replacer.formatUri(
            key
          )}`}`
        );
        return propertyRequest
  ...
  for (const key in thing.actions)
    setAction(thing, key, serviceUrl, td.title);
...
const setAction = (
  ...
  thing.setActionHandler(
    actionName,
    async (input = false) => {
      const url = `${serviceUrl}/${replacer.formatUri(
        actionName
      )}`;
      ...
      if (actionName.includes("delete-")) {
        ...
        const actionDeleteRequest = axios.delete(
          url,
          request
        );
        logger.info(
        ...
        logger.info(url);
        const actionPostRequest = axios.post(url, input, {
          headers,
        });
        logger.info(
          `performing ${actionName} action of ${title} thing to ${url} - input: ${JSON.stringify(
