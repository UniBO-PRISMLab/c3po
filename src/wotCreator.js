...
const createThing = async (descriptor) => {
    ...
    for (const key in thing.properties) {
        logger.info(`setting ${key} property handler of ${td.title} Thing to ${descriptor.serviceUrl + '/' + key.replace(/--/g, "/")}`);

        if (!thing.properties[key].readOnly) {
            thing.setPropertyWriteHandler(key, async (input) => {
                const headers = headerFactory.put()
                const propertyRequest = axios.put(descriptor.serviceUrl + '/' + key.replace(/--/g, "/"), input, { headers });
                logger.info(`performing PUT request for ${key} property of ${td.title}  at ${descriptor.serviceUrl + '/' + key.replace(/--/g, "/")}`);
        ...
        if (!thing.properties[key].writeOnly)
            thing.setPropertyReadHandler(key, async () => {
                ...
                const propertyRequest = axios.get(descriptor.serviceUrl + '/' + key.replace(/--/g, "/"), { headers });
                logger.info(`performing GET request for ${key} property of ${td.title} Thing at ${descriptor.serviceUrl + '/' + key.replace(/--/g, "/")}`);
                return propertyRequest.then(response => response.data).catch(resultHandler.errorHandler);
            });
    }

    for (const key in thing.actions)
        setAction(thing, key, descriptor.serviceUrl, td.title);
...
const setAction = (thing, actionName, serviceUrl, title) => {
    thing.setActionHandler(actionName, async (input) => {
        ...
        if (actionName.includes("delete")) {
            ...
            logger.info(`setting ${actionName} action handler of ${title} thing to ${url}`);
            return actionDeleteRequest.then(response => response.data).catch(resultHandler.errorHandler);
        } else {
            const actionPostRequest = axios.post(url, input, { headers });
            logger.info(`performing ${actionName} action of ${title} thing to ${url} - input: ${JSON.stringify(input)}`);
            return actionPostRequest.then(response => {
